...
Promise.resolve().then(() => {
  console.log('[...带入下一个阶段] promise 回调1')
...
readFile('../package.json','utf-8',data=>{
  ...
  setImmediate(() => {
    ...
    Promise.resolve().then(() => {
      console.log('[...带入下一个阶段] promise 回调2')
      process.nextTick(()=>{
        console.log('[...带切入下一阶段] nextTick 回调2 增加的 nextTick 回调5')
      })
    })
    .then(() => {
      console.log('[...带入下一个阶段] promise 回调3')
    })
  })
})
...
setImmediate(() => {
  ...
  process.nextTick(() => console.log('[...带入下一个阶段] promise 回调6 增加的 nextTick 回调7'))
  console.log('[...带入下一个阶段] 这块正在同步阻塞的读一个大文件')
  const video = readFileSync('../../video.mp4')
  process.nextTick(()=>console.log('[...带入下一个阶段] immediate 回调5 增加的 nextTick 回调 7'))
  readFile('../package.json','utf-8', data => {
...
setTimeout(() => {
  ...
  process.nextTick(()=>{
    console.log('[...带切入下一阶段] nextTick 回调5')
  })
}, 0);
